...
本工具基于**文件名、元数据及时长**进行模糊相似度匹配，检测结果仅供参考，**存在误判的可能性**：

- 同名但内容不同的歌曲（如不同版本、Live 版、翻唱）可能被误判为重复
- 通过传递性关联（间接匹配）纳入组的文件（以 `~` 前缀标注）尤其需要人工核实
- 删除操作会将文件移至系统回收站，但仍建议在批量删除前自行备份重要文件

**请在确认无误后再执行删除，作者对因误删导致的数据丢失不承担任何责任。**
